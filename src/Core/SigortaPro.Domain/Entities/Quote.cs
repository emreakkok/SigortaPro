...
public class Quote : BaseEntity, IAggregateRoot
{
    ...
    // detayında prim dökümü bu seçim + saklı veri üzerinden deterministik yeniden hesaplanır.
    public CoveragePackage CoveragePackage { get; private set; } = CoveragePackage.Standart;

    // Hasar geçmişi çarpanı (varsayılan 1.00 = etkisiz). Yalnızca yenileme tekliflerinde müşterinin
    // önceki dönem hasar geçmişine göre 1.00'ın üzerine set edilir; prim dökümünün deterministik yeniden
    // hesabında CoveragePackage gibi saklı bir girdi olarak kullanılır.
    // (LEGACY): Bu alan artık YENİ tekliflerde kullanılmaz — hasar geçmişi tek bir Bonus-Malus
    // basamağıyla (PricingSnapshot.NoClaimTier) fiyatlanır. Alan ÖNCESİ oluşturulmuş yenileme
    ...
    // YENİLEME İNDİRİMİ çarpanı. Varsayılan 1.00 = indirim yok. Yalnızca YENİLEME teklifleri,
    ...
    // (deterministik yeniden hesap). Yeni (yenileme dışı) tekliflerde 1.00 kalır → döküm değişmez.
    public decimal RenewalDiscountFactor { get; private set; } = 1.00m;

    // Sağlıkta "başkası adına" teklifte sigortalanan kişi (null = poliçe sahibi kendisi).
    // Owned/gömülü değer; fiyatlamanın deterministik yeniden hesabında yaş bu kişiden türetilir.
    public InsuredPerson? InsuredPerson { get; private set; }

    // Teklifin fiyatlandığı tarife versiyonu. Teklif oluşturulurken o an yürürlükte olan versiyon
    ...
    // Teklifin fiyatlandığı andaki risk GİRDİLERİ (sürücü/sigortalı yaşı, araç yaşı, motor gücü,
    // risk ili, bina yaşı, m², deprem bölgesi, sigara beyanı). tarifeyi sabitler; bu da girdiyi
